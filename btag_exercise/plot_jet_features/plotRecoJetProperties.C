...
void makePlot(const char *fileName, const char *outputName)
{
...
int plotRecoJetProperties(const char *filePath, const char *outputName)
{
  printf("Using file at: %s \n", filePath);

  makePlot(filePath, outputName);
  return 0;
}
